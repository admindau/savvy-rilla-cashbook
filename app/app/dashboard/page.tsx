...
export default function Dashboard() {
  ...
  async function fetchIdsAndCount(q: string) {
    let catIds: string[] = [];
    if (q) {
      const { data: catData } = await supabase.from("categories").select("id").ilike("name", `%${q}%`);
      catIds = (catData || []).map((x) => x.id);
    }
    ...
    const { count: total } = await tr;
    setCount(total || 0);
    return catIds;
  }

  // define load BEFORE all callers
  async function load(p: number, q: string) {
    ...
    const [{ data: aData }, { data: cData }, { data: trData }] = await Promise.all([
  ...
  const startEdit = (t: Tx) => { setEditingId(t.id); setEditDraft({ ...t }); };
  ...
  const saveEdit = async () => {
    ...
    setEditingId(null); setEditDraft({}); load(page, query);
  };
  ...
  return (
    <RequireAuth>
      <div className="grid gap-6">
        ...
        {/* Balances with independent toggles */}
        <div className="grid md:grid-cols-2 gap-4">
          <div
            className={`card cursor-pointer transition ${showLifetimeTotals ? "ring-2 ring-cyan-400 shadow-lg shadow-cyan-400/40" : "hover:bg-white/10"}`}
          ...
          <div
            className={`card cursor-pointer transition ${showMonthlyTotals ? "ring-2 ring-cyan-400 shadow-lg shadow-cyan-400/40" : "hover:bg-white/10"}`}
        ...
        <div className="card">
          ...
          <form
            action={async (fd: FormData) => { await addAccount(fd); }}
        ...
        <div className="card">
          ...
          <form
            action={async (fd: FormData) => { await addCategory(fd); }}
            className="grid md:grid-cols-3 gap-3"
          >
            <input name="name" placeholder="Name" required className="input" />
            <select name="kind" className="input">
