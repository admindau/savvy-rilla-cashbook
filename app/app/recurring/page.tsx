...
type Recurring = { id: string; category_id: string; amount: number; currency: string; frequency: "daily"|"weekly"|"monthly"; start_date: string; next_run: string|null; user_id?: string };
...
export default function RecurringPage() {
  ...
  const fetchAll = async () => {
    ...
    let q = supabase.from("recurring").select("*").eq("user_id", user_id).order("start_date", { ascending: true })
      .range(page * pageSize, page * pageSize + pageSize - 1);

    if (query) {
      const match = (cData || []).filter((c) => c.name.toLowerCase().includes(query.toLowerCase()));
      const catIds = match.map((c) => c.id);
      if (catIds.length) q = q.in("category_id", catIds);
      else { setRules([]); return; }
    }
  ...
  useEffect(() => { if (!loading) fetchAll(); }, [page, query, loading]); // eslint-disable-line

  if (loading) return <div>Loading FX rates...</div>;

  const addRule = async (fd: FormData) => {
    const { data: userData } = await supabase.auth.getUser();
    const user_id = userData?.user?.id; if (!user_id) { setToast({ message: "❌ No logged-in user", type: "error" }); return; }
  ...
  const applyRule = async (_id: string) => {
    setToast({ message: "✅ Rule applied (TX creation can be plugged here)", type: "success" });
  };

  // Chart
  const donut = {
    labels: rules.map((r) => cats.find((c) => c.id === r.category_id)?.name ?? "—"),
    datasets: [{ data: rules.map((r) => convert(Number(r.amount), r.currency, chartCurrency)), backgroundColor: rules.map((_, i) => colors[i % colors.length]) }],
  };
    ...
    <RequireAuth>
      <div className="grid gap-6">
        ...
        <div className="card">
          ...
          <form
            onSubmit={(e: FormEvent<HTMLFormElement>) => { e.preventDefault(); addRule(new FormData(e.currentTarget)); (e.currentTarget as HTMLFormElement).reset(); }}
          ...
          >
            ...
            <select name="frequency" className="input"><option value="daily">daily</option><option value="weekly">weekly</option><option value="monthly">monthly</option></select>
        ...
        <div className="card flex items-center justify-between gap-3">
          ...
          <input className="input max-w-md" placeholder="🔍 Search by category..." value={query} onChange={(e) => { setPage(0); setQuery(e.target.value); }} />
        </div>
        ...
        <div className="card overflow-x-auto">
          ...
          <table className="w-full text-sm">
            <thead className="text-white/70">
              <tr><th>Category</th><th>Amount</th><th>Currency</th><th>Frequency</th><th>Start</th><th>Next Run</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {rules.map((r) => {
                const name = cats.find((c) => c.id === r.category_id)?.name ?? "—";
                      ...
                      <>
                        ...
                        <td>
                          <select
                            ...
                            onChange={(e) => setEditDraft((d) => ({ ...d, frequency: e.target.value as "daily"|"weekly"|"monthly" }))}
                          >
                        ...
                        <td><input type="date" className="input" value={String(editDraft.start_date)} onChange={(e) => setEditDraft((d) => ({ ...d, start_date: e.target.value }))} /></td>
                        <td>{r.next_run ?? "—"}</td>
                        <td>
          ...
          <div className="flex justify-between items-center mt-3">
            ...
            <button disabled={rules.length<pageSize} className="btn" onClick={()=>setPage((p)=>p+1)}>Next</button>
          </div>
        </div>

        {/* Chart with FX toggle */}
        ...
        <div className="card"><h3 className="font-semibold mb-3">Recurring Totals ({chartCurrency})</h3><Doughnut data={donut}/></div>
