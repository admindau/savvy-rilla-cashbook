...
export default function BudgetsPage() {
  ...
  const legendItems = useMemo(() => {
    return categories.map(c => ({ id: c.id, name: c.name, color: colorForId(c.id) }));
  }, [categories]);
  ...
  return (
    <RequireAuth>
      <div className="grid gap-6">
        ...
        <div className="card">
          <h2 className="font-semibold mb-2">Category Legend</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 text-sm text-white/80">
            {legendItems.map(item => (
              <div key={item.id}><span className="legend-dot" style={{backgroundColor:item.color}}></span>{item.name}</div>
            ))}
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {budgets.map(b => {
            ...
            const col = colorForId(b.category_id);
            return (
