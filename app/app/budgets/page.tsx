...
import RequireAuth from '@/components/RequireAuth'; import { supabase } from '@/lib/supabaseClient'; import { useEffect, useMemo, useState } from 'react'; import { fmt } from '@/lib/format'; import { colorForId } from '@/lib/palette';
...
type Tx={category_id:string|null; amount:number; kind:string; currency:string; tx_date:string;};
export default function BudgetsPage(){
  ...
  const fetchAll = async () => {
    ...
    const t = await supabase.from('transactions').select('category_id, amount, kind, currency, tx_date').gte('tx_date', start).lte('tx_date', end);
    if (!c.error) setCategories(c.data as any); if (!b.error) setBudgets(b.data as any);
    if (!t.error) { const map: Record<string, number> = {};
  ...
  const addBudget = async (form: FormData) => { const category_id = String(form.get('category_id')); const limit_amount = Number(form.get('limit_amount'));
    const currency = String(form.get('currency')); const monthDate = month + '-01'; const { error } = await supabase.from('budgets').insert({ category_id, limit_amount, currency, month: monthDate });
